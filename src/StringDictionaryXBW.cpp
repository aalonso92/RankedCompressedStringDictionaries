...
StringDictionaryXBW::StringDictionaryXBW(IteratorDictString *it, bool sparse_bitsequence, int bparam)
{
	this->type = DXBW;
	this->elements = 0;
	this->maxlength = 0;
	...
	xbw = new XBW();
	xbw->build(len, mapping, alpha, last, A, sparse_bitsequence, bparam);
}
...
{
	return new IteratorDictStringXBW(NULL, 0, 2, xbw->select_A[2]-1, xbw, maxlength);
}
