...
void useBuild()
{
	...
	cerr << " \t <compress_table> : '1' plain (HASH); '2' compressed (HASHB); '3' re-compressed (HASHBB)." << endl;
	cerr << " \t <compress_str> : 'h' for Huffman; 'r' for RePair compression (strings)." << endl;
		...
		switch (type)
		{
			...
			case 1:
			{
				...
				if (in.good())
				{
					...
					uint compresst = atoi(argv[2]);
					uint overhead = atoi(argv[4]);
					...
					{
						// Huffman compression
						dict = new StringDictionaryHASHHF(it, lenStr, overhead, compresst);
						filename += string(".hashhf");
					}
					else
					{
						// RePair compression
						dict = new StringDictionaryHASHRPF(it, lenStr, overhead, compresst);
			...
			case 7:
			{
				...
				if (in.good())
				{
					...
					{
						// Plain bitmaps
						dict = new StringDictionaryXBW(it, false, atoi(argv[3]));
						delete it;

						filename = string(argv[5])+string(".")+string(argv[3])+string(".rg.xbw");
					}
					else if (argv[2][0] == 'c')
					{
						// Compressed bitmaps
						dict = new StringDictionaryXBW(it, true, atoi(argv[3]));
						delete it;

						filename = string(argv[5])+string(".")+string(argv[3])+string(".rrr.xbw");
					}
					else
					{
						checkFile();
